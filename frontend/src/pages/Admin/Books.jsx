...
const handleViewPdf = async (book, displayStatus) => {
    ...
    try {
        ...
        try {
            // Validar que el pdfBase64 no esté vacío
            const cleanBase64 = pdfBase64.trim();
            
            if (!cleanBase64) {
                throw new Error('El PDF recibido está vacío');
            }

            const pdfBlob = b64toBlob(cleanBase64, 'application/pdf');
            ...
            if (!newWindow) {
                displayStatus('No se pudo abrir el PDF. Por favor, permite las ventanas emergentes.', 'warning');
            }
            ...
            console.error('Error al convertir base64:', conversionError);
            displayStatus(
                `Error al procesar el PDF: ${conversionError.message}. Puede que el formato de datos sea incompatible.`,
                'error'
            );
        }
...
const AdminDashboard = () => {
    ...
    // --- BÚSQUEDA GLOBAL ---
    // Función de búsqueda con useCallback para evitar recreaciones
    ...
    // Debounce para búsquedas del usuario
    useEffect(() => {
    ...
    }, [searchTerm, performSearch, initialLoad]);
    
    // Handler para ver el PDF
